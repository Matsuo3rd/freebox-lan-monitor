...
main() {
	...
	exec &> >(tee -a "${LOG_DIR}/freebox-lan-monitor.log")
	
	log "Starting freebox-lan-monitor"

	if [[ ! -z "$MOSQUITTO_PUB" ]]; then
		if [ -f "${FREEBOX_API_CONFIG_DIR}/mqtt_config.json" ]; then
			log "Loading MQTT config in ${FREEBOX_API_CONFIG_DIR}/mqtt_config.json"
...
requestAuthorization () {
	# 1ere etape: on recupere le token de l'appli. Uniquement au 1er passage.
	...
	if [ ! -f ${FREEBOX_API_TMP_DIR}/status.json ] ; then
		...
		do
			...
			case $status in
		    granted)
		      log "The App Token is valid and can be used to open a session"
		      ;;
		    pending)
		    ...
		    denied)
		      ...
		      ;;
		    *)
		      error "Unexpected app authorization status error: $status. Aborting."
		      cat ${FREEBOX_API_TMP_DIR}/status.json
		      exit 1
		      ;;
		  esac
		done
...
login() {
	...
	#${CURL} --trace-ascii -d var=val ${FREEBOX_API_BASE_URL}/login/session/

	#cat ${FREEBOX_API_TMP_DIR}/credentials.json
...
quit() {
	log "Stoping freebox-lan-monitor"
	...
	while [[ $# -gt 0 ]]; do
	  case $1 in
		...
		-gstate|--get-state)
	      ...
	      ;;
	    -*|--*)
	      error "freeboxPlayerRemote Unknown option $1"
	      exit 1
	      ;;
	    *)
